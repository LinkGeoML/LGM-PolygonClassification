...
def get_X_Y_data(filename):
	...
	for index, row in df.iterrows():
		...
		dian_vertex_count = len(dian_shape.bounds)
		pst_vertex_count = len(pst_shape.bounds)
		
		feature_list_dian = [area_dian, intersection_area_dian, perimeter_dian, dian_vertex_count]
		feature_list_pst = [area_pst, intersection_area_pst, perimeter_pst, pst_vertex_count]
	...
	#print(feature_array_dian[:,2])
	boundaries_mean_dian = np.ones(feature_array_dian.shape[0]) * np.mean(feature_array_dian[:,2])
	boundaries_mean_pst = np.ones(feature_array_pst.shape[0]) * np.mean(feature_array_pst[:,2])

	boundaries_var_dian = np.ones(feature_array_dian.shape[0]) * np.var(feature_array_dian[:,2])
	boundaries_var_pst = np.ones(feature_array_pst.shape[0]) * np.var(feature_array_pst[:,2])
